...
export default function CostCentersPage(): React.JSX.Element {
  const { data, loading, refetch } = useApi<CostCenterListResponse>('/cost-centers');
  ...
  const handleCreate = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      showToast.success('Cost center created');
      setShowCreate(false);
  ...
  return (
    <div className="space-y-6 p-4 lg:p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--color-foreground)]">Cost Centers (ศูนย์ต้นทุน)</h1>
          <p className="text-sm text-[var(--color-muted-foreground)]">Manage cost centers for expense analysis</p>
        </div>
      ...
      {showCreate && (
        <div className="rounded-lg border border-[var(--color-border)] p-4">
          <h2 className="mb-3 font-medium">Create Cost Center</h2>
          <form onSubmit={(e) => void handleCreate(e)} className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium">Code *</label>
              <input className={inputClass} value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} placeholder="CC-SALES" required maxLength={20} />
            </div>
            <div>
      ...
      {loading ? (
        ...
        <EmptyState
          context="cost-center-list"
          ctaLabel="Create First Cost Center"
        ...
        <div className="overflow-x-auto rounded-lg border border-[var(--color-border)]">
          <table className="w-full text-sm">
            ...
            <tbody>
              {centers.map((cc) => (
                <tr key={cc.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-accent)]/20">
                  <td className="px-4 py-3 font-mono font-medium text-sm">{cc.code}</td>
                  <td className="px-4 py-3 font-medium">{cc.nameEn}</td>
                  <td className="px-4 py-3 text-[var(--color-muted-foreground)]">{cc.nameTh}</td>
                  <td className="px-4 py-3 font-mono text-xs text-[var(--color-muted-foreground)]">{cc.parentId ?? '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${cc.isActive ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500'}`}>
                      {cc.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <Link href={`/cost-centers/${cc.id}/report`}>
